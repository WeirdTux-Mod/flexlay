...
class Level:
    ...
    def save_io(self, f):
        ...
        writer.write_comment("Generated by Flexlay Level Editor")
        ...
        if tileset_path is not None:
            clean_tileset = tileset_path.replace("\\", "/")
            
            data_dir = Config.current.datadir.replace("\\", "/")
            
            if not data_dir.endswith("/"):
                data_dir += "/"
                
            if clean_tileset.lower().startswith(data_dir.lower()):
                tileset_path = "/" + clean_tileset[len(data_dir):].lstrip("/")
            else:
                if "/data/" in clean_tileset:
                    tileset_path = "/" + clean_tileset.split("/data/", 1)[1]
                else:
                    tileset_path = clean_tileset
